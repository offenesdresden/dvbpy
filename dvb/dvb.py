...
def monitor(stop, offset=0, limit=10, city="Dresden", *, raw=False):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            raise requests.HTTPError(f"HTTP Status: {r.status_code}")
    ...
    return (
        ...
        if raw
            ...
            {"line": line, "direction": direction, "arrival": 0 if arrival == "" else int(arrival)}
...
def route(
    ...
    *,
    ...
    url = (
        "http://efa.faplino.de/dvb/XML_TRIP_REQUEST2"
        if eduroam
    ...
    try:
        ...
        print(
            "Failed to access VVO TripRequest. Connection timed out. Are you connected to eduroam?"
        )
        response = None
    except requests.RequestException as e:
    ...
    prettified = {
        ...
        "trips": [process_single_trip(single_trip) for single_trip in response["trips"]],
    }
...
def interchange_prediction(trip):
    ...
    interchanges = [
        (
            incoming["path"][-3],  # second to last point en route of the arriving route
            [
                int(x) / 1000000 for x in incoming["arrival"]["coords"].split(",")
            ],  # stop point of the arriving route
            [int(x) / 1000000 for x in outgoing["departure"]["coords"].split(",")],
        )  # start point of the departing route
    ...
    for i, interchange in enumerate(interchanges):
        ...
        b = np.subtract(
            interchange[2], interchange[1]
        )  # direction of the walk to the departing route
...
def find(search, eduroam=False, *, raw=False):
    ...
    url = (
        "http://efa.faplino.de/dvb/XML_STOPFINDER_REQUEST"
        if eduroam
    ...
    try:
        ...
        print(
            "Failed to access VVO StopFinder. Connection timed out. Are you connected to eduroam?"
        )
        response = None
    except requests.RequestException as e:
...
def find_return_results(stop):
    if "object" in stop and "coords" in stop["ref"]:
        ...
        # only city, no coords
        return {"name": stop["object"], "city": stop["posttown"]}
    elif "coords" in stop["ref"]:
        # only coords, no city
        return {"name": stop["name"], "coords": convert_coords(stop["ref"]["coords"])}
    else:
    ...
    if pintypes == "stop":
        return {
            ...
            "coords": pincoords_to_object(
                int(line.split("||")[1].split("|")[2]), int(line.split("||")[1].split("|")[3])
            ),
        ...
        return {
            ...
            "coords": pincoords_to_object(
                int(line.split("||")[1].split("|")[1]), int(line.split("||")[1].split("|")[2])
            ),
            "?": line.split("||")[1].split("|")[3],
        }
    elif (
        pintypes == "poi"
        or pintypes == "rentabike"
        or pintypes == "ticketmachine"
        or pintypes == "carsharing"
        or pintypes == "parkandride"
    ):
        return {
...
def process_address(line):
    try:
        return {"city": line.split("|")[0], "address": line.split("|")[1]}
