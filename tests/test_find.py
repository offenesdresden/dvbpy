...
class TestFind:
    ...
    def test_stop_with_city(self, mocked_responses: object) -> None:
        mock_get(mocked_responses, "tr/pointfinder", fixture="pointfinder.json")  # type: ignore[arg-type]
        results = dvb.find("Helmholtz")
        assert isinstance(results, list)
    ...
    def test_raw_returns_dict(self, mocked_responses: object) -> None:
        mock_get(mocked_responses, "tr/pointfinder", fixture="pointfinder.json")  # type: ignore[arg-type]
        result = dvb.find("Helmholtz", raw=True)
        assert isinstance(result, dict)
        assert "Points" in result

    def test_empty_results(self, mocked_responses: object) -> None:
        mock_get(  # type: ignore[arg-type]
            mocked_responses,
            "tr/pointfinder",
            body={"PointStatus": "List", "Status": {"Code": "Ok"}, "Points": []},
        )
        results = dvb.find("nonexistent")
        assert isinstance(results, list)
        assert len(results) == 0

    def test_api_error(self, mocked_responses: object) -> None:
        mock_get(  # type: ignore[arg-type]
            mocked_responses,
            "tr/pointfinder",
    ...
    def test_connection_error(self, mocked_responses: object) -> None:
