...
class TestLines:
    def test_parses_lines(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "stt/lines", fixture="lines.json")  # type: ignore[arg-type]
        results = client.lines("33000742")
    ...
    def test_second_line(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "stt/lines", fixture="lines.json")  # type: ignore[arg-type]
        results = client.lines("33000742")
        assert isinstance(results, list)
        line = results[1]
    ...
    def test_resolves_stop_name(self, mocked_responses: object, client: Client) -> None:
        ...
        mock_post(mocked_responses, "stt/lines", fixture="lines.json")  # type: ignore[arg-type]
        results = client.lines("Helmholtzstraße")
        assert isinstance(results, list)
        assert len(results) == 2

    def test_raw_returns_dict(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "stt/lines", fixture="lines.json")  # type: ignore[arg-type]
        result = client.lines("33000742", raw=True)
