...
class TestRouteChanges:
    def test_parses_changes(self, mocked_responses: object) -> None:
        mock_post(mocked_responses, "rc", fixture="route_changes.json")  # type: ignore[arg-type]
        results = dvb.route_changes()
    ...
    def test_validity_periods(self, mocked_responses: object) -> None:
        mock_post(mocked_responses, "rc", fixture="route_changes.json")  # type: ignore[arg-type]
        results = dvb.route_changes()
        assert isinstance(results, list)
    ...
    def test_multiple_line_ids(self, mocked_responses: object) -> None:
        mock_post(mocked_responses, "rc", fixture="route_changes.json")  # type: ignore[arg-type]
        results = dvb.route_changes()
        assert isinstance(results, list)
    ...
    def test_raw_returns_dict(self, mocked_responses: object) -> None:
        mock_post(mocked_responses, "rc", fixture="route_changes.json")  # type: ignore[arg-type]
        result = dvb.route_changes(raw=True)
