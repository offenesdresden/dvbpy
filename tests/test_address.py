...
class TestAddress:
    def test_returns_stop(self, mocked_responses: object) -> None:
        ...
        result = dvb.address(51.03, 13.73)
        assert result is not None
        assert not isinstance(result, dict)
        assert result.name == "Helmholtzstraße"
        assert result.id == "33000742"

    def test_returns_none_when_empty(self, mocked_responses: object) -> None:
        mock_get(  # type: ignore[arg-type]
            mocked_responses,
            "tr/pointfinder",
            body={"PointStatus": "List", "Status": {"Code": "Ok"}, "Points": []},
        )
        result = dvb.address(0.0, 0.0)
    ...
    def test_raw_returns_dict(self, mocked_responses: object) -> None:
        ...
        result = dvb.address(51.03, 13.73, raw=True)
