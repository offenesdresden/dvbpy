...
class TestPins:
    def test_parses_pins(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "map/pins", fixture="pins.json")  # type: ignore[arg-type]
        results = client.pins(51.0, 13.7, 51.1, 13.8)
        assert isinstance(results, list)
        assert len(results) == 3

    def test_stop_pin(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "map/pins", fixture="pins.json")  # type: ignore[arg-type]
        results = client.pins(51.0, 13.7, 51.1, 13.8)
        assert isinstance(results, list)
    ...
    def test_platform_pin(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "map/pins", fixture="pins.json")  # type: ignore[arg-type]
        results = client.pins(51.0, 13.7, 51.1, 13.8)
        assert isinstance(results, list)
    ...
    def test_park_and_ride_pin(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "map/pins", fixture="pins.json")  # type: ignore[arg-type]
        results = client.pins(51.0, 13.7, 51.1, 13.8)
        assert isinstance(results, list)
    ...
    def test_raw_returns_dict(self, mocked_responses: object, client: Client) -> None:
        mock_post(mocked_responses, "map/pins", fixture="pins.json")  # type: ignore[arg-type]
        result = client.pins(51.0, 13.7, 51.1, 13.8, raw=True)
        assert isinstance(result, dict)
        assert "Pins" in result

    def test_empty_pins(self, mocked_responses: object, client: Client) -> None:
